...
resultan mas convenientes para los cálculos de simulación \cite{Steyn2011}. La ecuación diferencial cinemática de los cuaterniones está descrita en la  \textbf{eq.(\ref{eq:quaterniones})}: 
...
Finalmente, realizando la transformada de Laplace de las ecuaciones \textbf{(\ref{eq:armadura})} a \textbf{(\ref{eq:constante_torque})} se pueden obtener las funciones de transferencia para la corriente $i(s)$, $\omega(s)$ y el torque del motor $T_m(s)$. 
...
\begin{figure}[h]
	...
	\end{center}
	\caption{Diagrama de bloques RW.}
	\label{fig:bloquesRW}
	\textit{}
\end{figure}
...
% 	Imagen de toma de resistencia
% Diagrama de bloques

\begin{figure}[h]
	\begin{center}
		\includegraphics[scale=0.5]{imagenes/modelo_dinamico/bloquesMgt.PNG}\\
	\end{center}
