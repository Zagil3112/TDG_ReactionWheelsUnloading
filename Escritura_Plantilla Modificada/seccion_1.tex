...
\begin{table}[!h]
	...
	\begin{tabular}{r|lllll|l}
		\multicolumn{1}{l|}{\textbf{ID}} & \textbf{Controlador}                  & \mbox{\boldmath$e_{ref}$}   & \mbox{\boldmath$\Delta h$} & \mbox{\boldmath$P_{mgt}$}   & \mbox{\boldmath$P_{Rw}$}    & \textbf{Total}      \\ 
		\hline
		1                       & PID $\Delta h$ & 0.00094 &0.0039& 0.04033& 2.01 &2.06  \\
...
\end{table}
% Detumbling %Nadir
No obstante, en estos escenarios cabe destacar que el PID presentó mejor desempeño en el \textit{Detumbling}, con una notoria disminución del error de referencia y el consumo de potencia de los magnetorquers. En cambio, para el apuntamiento en el Nadir, se presentó un desempeño muy similar para todos los controladores en cada una de las órbitas. Así mismo, es de notar que en este escenario, el momento angular de las RW, este se estabiliza en un valor 3 veces mayor con el método de $k\Delta h$ respecto al método de desaturación del controlador cascada \textbf{(Fig. \ref{fig:nadir})}, donde el primero presenta mayores oscilaciones. Sin embargo, estos valores de estabilización son del orden de $10^{-5}$. 
...
% Nadir cambio de coordenadas

%Estimación de perturbaciones por fricción para banco de pruebas 
