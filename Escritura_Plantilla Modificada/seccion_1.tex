...
\begin{figure}[!ht]
	...
	\end{center}
	\caption{ Marco de referencia centrado en el cuerpo}
...
\begin{figure}[!ht]
	\begin{center}
		\includegraphics[scale=0.7]{imagenes/marco_teorico/orbital_cuerpo.PNG}\\
	\end{center}
	\caption{ Marco de referencia centrado en el cuerpo y orbital}
...
\end{figure}
 ...
 Debido a que el problema discutido en este trabajo se restringió para solo un grado de libertad, solo basta realizar una rotacion de un ángulo $\phi$ en el eje $Z_o$ del marco orbital (ORF) para pasar al marco del cuerpo (BRF). Lo anterior puede lograrse con la matriz de transformación $R_{B}^{O}$:
$$
...
%Los ángulos de Euler pueden provocar singularidades en determinados ángulos cuando se utilizan en cálculo numérico.
%
%%gimbal lock
%
%Los parámetros simétricos de Euler (cuaternión) son una mejor representación sin singularidades
%y sin funciones trigonométricas en la matriz de transformación \cite{wertz1978spacecraft}. 
%
%El cuaternión se utiliza durante el cálculo numérico.Dado que la representación quaternion no tiene una representación física fácilmente interpretable, los ángulos de Euler se utilizarán
%en el presente estudio para representar la actitud durante las simulaciones y los resultados de las pruebas.
%
%El conjunto de cuaterniones consta de cuatro parámetros, que se definen en términos del eje de Euler
%de Euler. El teorema de rotación del eje propio de Euler establece que la rotación finita de un cuerpo rígido
%con un punto fijo puede expresarse mediante una única rotación alrededor de algún eje fijo [14].
%
%Este eje fijo, el eje de Euler, se representa mediante un vector unitario en la IRF o BRF con
%componentes ¯e = (ex, ey, ez).
%
% Los cuatro parámetros simétricos de Euler, o cuaterniones, están
%definidos en las ecuaciones (2.2.5) a (2.2.8), donde es el ángulo de rotación sobre el eje de Euler
