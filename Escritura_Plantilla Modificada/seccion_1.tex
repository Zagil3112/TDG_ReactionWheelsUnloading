...
\begin{equation}\label{eq:modelo_dinmico_linealizado}
	\dot{\omega_b}=I^{-1}\left[S(I\omega)-S(\omega)I+S(h_w)\right]\tilde{\omega}-I^{-1}S(\omega)\tilde{h_w}+I^{-1}\left(T_{RW}+T_{mgt}\right)
\end{equation}
...
Donde $g$ se conoce como vector de Gibbs y corresponde a las primeras 3 componentes vectoriales de un cuaternion. 
%	q = \left[\begin{array}{c}
	%	\eta\\
	%	\epsilon_1\\
	%	\epsilon_2\\
