...
\begin{equation}\label{eq:modelo_dinamico_linealizado}
	\boldsymbol{\dot{\omega_b}}=\boldsymbol{I}^{-1}\left[S(I\bar{\omega}-S(\bar{\omega})I+S(\bar{h_w})\right]\tilde{\omega}-I^{-1}S(\bar{\omega})\tilde{h_w}+I^{-1}\left(T_{RW}+T_{mgt}\right)
\end{equation}
...
% Implementacion 
Ademas de verificar la ubicación de los polos, un ajuste progresivo del controlador fue realizado al evaluar individualmente sus componentes derivativa, integral y proporcional, hasta dar con unas ganancias que permitieran la estabilidad con un bajo tiempo de estabilización y poca sobreimpulso máximo. 
