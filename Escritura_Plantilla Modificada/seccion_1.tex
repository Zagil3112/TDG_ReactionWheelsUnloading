...
\begin{table}[h]
	...
	\begin{center}
		\begin{tabular}{ c c c}
			...
			$R$ &  Inductancia & $4.91\cdot10^{-7}$ \\ 
			$K_t$ &  Constante de torque & $2.27\cdot10^{-6}$ \\ 
			$K_e$ &  Constante de campo& $1.33\cdot10^{-8}$ \\ 
			$B$ &  Coeficiente de fricción& $1.33\cdot10^{-8}$ \\ 
			$T_{max}$ & Torque máximo & $1.33\cdot10^{-8}$ \\
			$\omega_{max}$ & RPM máximas & $1.33\cdot10^{-8}$ \\ \hline
		\end{tabular}
	\end{center}
	\label{table:motorRW}
\end{table}
